...
function resolveErrorMessage(task: DownloadTask): string | null {
  if (task.error) {
    return task.error;
  }
...
function DownloadItemComponent({
  ...
  const isActive = task.state === "downloading" || task.state === "queued" || task.state === "waiting";
  ...
  const isIndeterminate = task.progressMode === "indeterminate" && task.state === "downloading";

  return (
    <article className="rounded-xl border border-border bg-card p-3.5 shadow-sm transition-all duration-200">
      ...
      {/* Progress bar */}
      {(task.state === "downloading" || task.state === "paused" || task.state === "queued" || task.state === "waiting") && (
        <div className="mt-2.5">
          ...
          {typeof task.queuePosition === "number" && task.queuePosition > 0 && (
            <p className="mt-1 text-[11px] text-muted-foreground">Queue position: {task.queuePosition}</p>
          )}
        </div>
      )}
      ...
      <div className="mt-2.5 flex flex-wrap items-center gap-1.5">
        ...
        {(task.state === "canceled" || task.state === "completed" || task.state === "failed" || task.state === "evicted") && (
