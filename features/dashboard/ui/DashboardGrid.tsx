...
import { FolderCard } from "@/features/file/ui/folder/FolderCard";
import { ListRow } from "@/features/file/ui/folder/ListRow";
...
const SORT_LABELS: Record<DashboardToolbarSortKey, string> = {
  recent: "Recent",
  name: "Name",
  metric: "Credits",
};
...
function resolveAssignment(
  ...
  return {
    tagIds: routeAssignment?.tagIds ?? [],
    starred: Boolean(routeAssignment?.starred),
  };
}
...
export function DashboardGrid({ showSharedChrome = true }: DashboardGridProps) {
  ...
  // Sync shared view mode when the setting changes externally.
  useEffect(() => {
  ...
  // Sync shared sort key when the setting changes externally.
  useEffect(() => {
  ...
  const folders = useMemo<DashboardFolder[]>(
    () =>
      courses
        ...
        const assignmentState = resolveAssignment(assignments, routeId);
        return {
          id: routeId,
          routeId,
  ...
  const folderTagPreviewById = useMemo(() => {
    ...
    for (const folder of folders) {
      const previews: Array<{ id: string; name: string; color: string }> = [];
            ...
            <div className="flex items-center gap-2">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start" className="w-36">
                  {(Object.keys(SORT_LABELS) as DashboardToolbarSortKey[]).map((key) => (
