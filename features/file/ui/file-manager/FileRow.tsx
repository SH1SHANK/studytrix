...
function FileRowComponent({
  ...
  useEffect(() => {
    const onBatchQueued = (event: Event) => {
      const payload = (event as CustomEvent<{ fileIds?: string[] }>).detail;
      const ids = Array.isArray(payload?.fileIds) ? payload.fileIds : [];
      if (!ids.includes(id)) {
        return;
      }
      setShowOfflinePulse(true);
      window.setTimeout(() => setShowOfflinePulse(false), 400);
    };

    window.addEventListener("studytrix:offline-batch-queued", onBatchQueued as EventListener);
    return () => {
      window.removeEventListener("studytrix:offline-batch-queued", onBatchQueued as EventListener);
    };
  }, [id]);
      ...
      <AnimatePresence mode="wait">
        {showOfflinePulse ? (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            transition={{ duration: 0.4 }}
            className="pointer-events-none absolute inset-0 z-20 rounded-xl bg-primary/10"
          />
        ) : null}
      </AnimatePresence>
      {/* ── Swipe Action Panel ─────────────────────────────── */}
