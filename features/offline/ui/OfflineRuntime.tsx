...
import { getProviderFileBlob } from "@/features/offline/offline.db";
...
async function runOfflineIntegrityPass(): Promise<void> {
  const provider = getActiveProvider();
  if (!provider) {
    return;
  }

  const accessOk = await provider.testAccess();
  if (!accessOk) {
    return;
  }

  const tasks = Object.values(useDownloadStore.getState().tasks);
  const removeOffline = useOfflineIndexStore.getState().removeOffline;
  for (const task of tasks) {
    if (task.state !== "completed" || task.kind === "folder") {
      continue;
    }

    try {
      const blob = await getProviderFileBlob(task.fileId);
      if (blob) {
        continue;
      }
      ...
      useDownloadStore.getState().updateTask(task.id, {
        state: "evicted",
        error: "Cached file was removed from storage.",
        errorCode: "UNKNOWN",
        progress: 0,
        updatedAt: Date.now(),
      });
      await removeOffline(task.fileId);
    } catch {
    }
  }
}
