...
export function AssignTagsDrawer() {
  ...
  const createAndAssignTag = async () => {
    ...
    try {
      ...
      const color = tagColorPalette[Math.floor(Math.random() * tagColorPalette.length)];
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && closeDrawer()}>
      <DialogContent className="fixed inset-x-0 bottom-0 top-auto mx-auto flex max-h-[85dvh] w-full max-w-lg flex-col overflow-hidden rounded-t-3xl border-t border-border/50 bg-card/95 p-0 shadow-2xl backdrop-blur-xl border-border/80 bg-background/95 translate-x-0 translate-y-0 sm:inset-auto sm:left-1/2 sm:top-1/2 sm:max-h-[80dvh] sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-3xl sm:border">
        ...
        <div className="px-6 py-2">
          <Input
            ...
            className="h-12 rounded-xl border-border bg-muted/50 shadow-inner focus-visible:ring-indigo-500/50 border-border bg-card/50"
          />
        </div>
        ...
        <div className="flex flex-1 flex-col gap-1 overflow-y-auto px-4 pb-6 pt-2">
          {filteredTags.length === 0 && searchQuery.trim() && (
            <button
              ...
              className="group flex w-full items-center gap-3 rounded-xl p-3 text-left transition-colors hover:bg-muted hover:bg-card"
            >
          ...
          {filteredTags.map((tag) => {
            ...
            return (
              <button
                ...
                className="group flex w-full items-center justify-between rounded-xl px-2 py-2.5 transition-colors hover:bg-muted focus-visible:bg-muted focus-visible:outline-none hover:bg-muted/60 dark:focus-visible:bg-muted/60"
              >
                <div className="flex items-center gap-3">
                  <div className="relative flex size-8 items-center justify-center rounded-lg border border-border/50 bg-card transition-colors border-border/50 bg-card">
                    {isAssignedToAll ? (
                      ...
                      <div className="size-5 rounded-md border-2 border-border transition-colors group-hover:border-border group-hover:border-border" />
                    )}
                  </div>
                  <span
                    className="inline-flex items-center rounded-full border border-border/30 px-2 py-0.5 text-xs font-semibold"
